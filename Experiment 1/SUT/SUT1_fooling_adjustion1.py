...
@dataclass
class FoolingConfig:
    model_path: str = r"C:\Users\spaeth\Desktop\Projecte\Konferenzen\ASAB\ASAB\artifacts\sut1_mnist_classifier.keras"
    decoded_root_dir: str = r"../CVAutoEncoder/runs/cvae_mnist/cluster_sampling_by_label_new_area/"

    epsilon: float = 0.10
    m_samples_per_cluster: int = 1000

    output_dir: str = r"../CVAutoEncoder/runs/cvae_mnist/cluster_sampling_by_label_new_area/fooling_results"
...
def predict_labels_and_probs(
    ...
    if probs.ndim != 2:
        raise ValueError(
            f"Expected multiclass probabilities of shape (N, num_classes), got {probs.shape}"
        )
...
def main() -> None:
    base_model_path = r"C:\Users\spaeth\Desktop\Projecte\Konferenzen\ASAB\ASAB\artifacts\sut1_mnist_classifier.keras"

    base_decoded_root = r"../CVAutoEncoder/runs/cvae_mnist/cluster_sampling_by_label_new_area"
    base_output_root = r"../CVAutoEncoder/runs/cvae_mnist/cluster_sampling/fooling_results"

    # Epsilon von 0.0 bis 1.0 in 0.1 Schritten
    epsilons = np.arange(0.0, 1.01, 0.1)

    for eps in epsilons:
        eps_str = f"{eps:.2f}"
        epsilon_folder = f"epsilon_{eps_str}"

        #for label in range(0,10):
        decoded_root_dir = os.path.join(base_decoded_root, epsilon_folder, f"decoded_cluster_samples_new_area")
        output_dir = os.path.join(base_output_root, f"epsilon_{eps_str}")

        print(f"Running evaluation for epsilon={eps_str}")
        print(f"Input: {decoded_root_dir}")
        print(f"Output: {output_dir}")

        config = FoolingConfig(
            model_path=base_model_path,
            decoded_root_dir=decoded_root_dir,
            epsilon=eps,
